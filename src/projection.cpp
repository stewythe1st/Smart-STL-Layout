...
void projection::print_on_bmp(bitmap_image& bmp, int x_offset, int y_offset, int rot) {
	
	// Variables
	layout l = m_rotations[rot];

	// Draw on bitmap
	for (int x = 0; x < l.m_xsize; x++) {
		for (int y = 0; y < l.m_ysize; y++) {
			if (l.m_grid[x][y] && x + x_offset < (int)bmp.width() && y + y_offset < (int)bmp.height()) {
